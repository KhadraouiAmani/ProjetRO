...
# ======================
def generate_patterns(stock_length, pieces, kerf=5):
    patterns = [] 
    n = len(pieces) 
    ...
    for length, qty, name in pieces:
        ...
        current_length = 0 
        while True:
            added_len = length + (kerf if max_fit > 0 else 0)
            if current_length + added_len <= stock_length:
                max_fit += 1
    ...
    for counts in product(*[range(m+1) for m in max_counts]):
        ...
        total_length_parts = sum(counts[i] * pieces[i][0] for i in range(n))
        num_cuts = sum(counts)
        total_kerf = (num_cuts - 1) * kerf if num_cuts > 0 else 0
        
        final_length_used = total_length_parts + total_kerf
        
        if final_length_used <= stock_length:
            pattern = {i: counts[i] for i in range(n) if counts[i] > 0} 
...
class SolverThread(QThread):
    ...
    def run(self):
        ...
        max_stock_len = max([s[0] for s in self.stock_data]) if self.stock_data else 0
        ...
        for diam_val, demands in self.demands_list:
            ...
            impossible_pieces = []
            for (length, qty, name) in demands:
                if length > max_stock_len:
                    impossible_pieces.append(f"{name} ({length}mm)")
            
            if impossible_pieces:
                msg = ", ".join(impossible_pieces)
                results[key] = {"error": f"IMPOSSIBLE : Les pièces suivantes sont plus longues que le plus grand stock ({max_stock_len}mm) : {msg}"}
                continue
    ...
    def solve_diameter(self, diameter, demands):
        ...
        for stock_idx, (length, cost, avail) in enumerate(self.stock_data):
            ...
            for p_idx, pattern in enumerate(patterns):
                x_vars[stock_name][p_idx] = model.addVar(
                    vtype=GRB.CONTINUOUS, 
                    name=f"x_{stock_name}_{p_idx}"
                )
        ...
        for stock_idx, (length, cost, avail) in enumerate(self.stock_data):
            ...
            model.addConstr(stock_expr <= avail, f"Stock_Avail_{stock_name}")
        ...
        if model.status == GRB.OPTIMAL:
            ...
            return {"cost": None, "patterns": [], "stock_used": {}, "infeasible": True}
             ...
             return {"cost": None, "patterns": [], "stock_used": {}, "error": f"Solver Status: {model.status}"}
...
class CuttingStockApp(QWidget):
    ...
    def rebuild_demand_tabs(self):
        ...
        for k in range(num_diams):
            ...
            self.tabs.addTab(tab_widget, f"Diamètre {spin_diam.value()}mm")
    ...
    def start_solver(self):
        ...
        if (self.num_stocks_spin.value() >= 7 or total_types >= 15):
            reply = QMessageBox.question(self, "Avertissement", "Complexité élevée. Continuer ?",
        ...
        try:
            ...
            for idx, (spin_diam, spin_types, table) in enumerate(self.tab_widgets_list):
                ...
                try:
                    for r in range(table.rowCount()):
                        l_it = table.item(r, 0)
                        q_it = table.item(r, 1)
                        if not l_it or not q_it: raise ValueError(f"Ligne {r+1} : Données manquantes")
                        
                        l_val = float(l_it.text())
                        q_val = int(q_it.text())
                        
                        if l_val <= 0: raise ValueError(f"Ligne {r+1} : Longueur doit être > 0")
                        if q_val < 0: raise ValueError(f"Ligne {r+1} : Quantité ne peut pas être négative")
            ...
            self.result_text.setText("⏳ Vérification des contraintes et optimisation...")
            self.thread.start()
    ...
    def show_results(self, results):
        ...
        for key in sorted_keys:
            ...
            if res.get("infeasible"):
                text += "❌ Problème infaisable : Stock insuffisant pour satisfaire la demande.\n\n"
                continue
                
            if res['cost'] == 0:
                text += "ℹ️  Aucune demande active.\n\n"
                continue
            ...
            for stock_name, qty in sorted(res['stock_used'].items()):
                ...
                if qty > 0.001: 
                    text += f"  • {stock_name}: {qty:.2f} barres\n"
